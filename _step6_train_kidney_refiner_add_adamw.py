...
class Config:
    ...
    # 显存与梯度累加配置 (针对 RTX 4060 8GB 调优)
    BATCH_SIZE = 4               
    ACCUMULATION_STEPS = 8       # 4 x 8 = 等效 32 的 Batch Size
    LR = 1e-4                    
    NUM_EPOCHS = 20              
    NUM_WORKERS = 4              
    ...
    # 【新增】：将高分辨率图像强行缩放，拯救 Transformer 显存
    IMG_SIZE = (480, 270)  # (Width, Height)
...
def setup_logger(log_dir):
    ...
    logger = logging.getLogger("KidneyTrain")
...
def plot_loss_curves(train_losses, val_losses, result_dir):
    ...
    plt.plot(train_losses, label='Train ADD Loss (mm)', marker='o', alpha=0.7)
    plt.plot(val_losses, label='Val ADD Loss (mm)', marker='x', alpha=0.7)
    plt.title('Training and Validation Loss')
    plt.xlabel('Epoch')
    plt.ylabel('ADD Loss / L2 Distance (mm)')
...
# ==========================================
# 2. 离线 Pair Dataset：只读取 step5 预渲染好的 A/B，训练阶段不再在线渲染
# ==========================================
...
def compute_add_loss(pred_trans, pred_rot_6d, gt_T_A, gt_T_B, model_points, trans_scale=50.0):
    B = pred_trans.shape[0]
    
    # 1. 解析网络预测的【相对变换 T_rel】(从 A 到 B 的纠正量)
    R_rel_pred = compute_rotation_matrix_from_6d(pred_rot_6d) # (B, 3, 3)
    t_rel_pred = (pred_trans * trans_scale).unsqueeze(2)      # (B, 3, 1)
    ...
    # 3. 【数学核心】计算预测的最终绝对位姿 T_est = T_A * T_rel_pred
    # 物理意义：将相对修正量叠加到状态 A 上，以期望达到状态 B
    ...
    # 预测点云：P_est = R_est * P_obj + t_est
    pts_est = torch.bmm(R_est, pts_obj) + t_est
    # 真实目标点云：P_gt = R_B * P_obj + t_B
    pts_gt = torch.bmm(R_B, pts_obj) + t_B
    
    # 5. 计算 L2 几何物理距离 (mm)
    l2_dist = torch.norm(pts_est - pts_gt, p=2, dim=1)
    return l2_dist.mean()
    ...
    for epoch in range(start_epoch, cfg.NUM_EPOCHS):
        ...
        for batch_idx, (inputs, gt_T_A, gt_T_B) in enumerate(pbar):
            ...
            with torch.cuda.amp.autocast():
                ...
                # 传入 gt_T_A 和 gt_T_B 进行链式计算
                loss = compute_add_loss(out['trans'], out['rot'], gt_T_A, gt_T_B, model_points, trans_scale=cfg.TRANS_SCALE)
            ...
            pbar.set_postfix({'ADD(mm)': f"{true_loss_val:.2f}"})
        ...
        epoch_val_loss = 0.0
        with torch.no_grad():
            for inputs, gt_T_A, gt_T_B in tqdm(val_loader, desc="Validation"):
                inputs, gt_T_A, gt_T_B = inputs.to(device), gt_T_A.to(device), gt_T_B.to(device)
        ...
        logger.info(f"[结果] Train ADD: {avg_train_loss:.4f} mm | Val ADD: {avg_val_loss:.4f} mm")
        ...
        if avg_val_loss < best_val_loss:
            ...
            logger.info(f"[*] 新纪录！最佳权重已保存至: {best_path} (误差缩减至 {best_val_loss:.4f} mm)")
